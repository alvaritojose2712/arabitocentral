export default function serarchBar({
    ...
    return(
    <form onSubmit={e=>{e.preventDefault();selectCuentaPorPagarProveedorDetallesFun()}} className="mb-2">
        <div className="input-group">
            <span className={("btn btn-"+(cuentaporpagarAprobado==0?"sinapsis":""))} onClick={e=>setcuentaporpagarAprobado(0)}><i className="fa fa-clock-o"></i></span>
            <span className={("btn btn-"+(cuentaporpagarAprobado==1?"success":""))} onClick={e=>setcuentaporpagarAprobado(1)}><i className="fa fa-check"></i></span>
            
            <input type="text" className="form-control form-control-lg" placeholder={"Buscar en..."} onChange={e=>setqcuentasPorPagarDetalles(e.target.value)} value={qcuentasPorPagarDetalles} />
            ...
            <button type="button" className="btn btn-warning btn-sm" onClick={()=>selectCuentaPorPagarProveedorDetallesFun("reporte")}><i className="fa fa-print"></i></button>
            <button type="submit" className="btn btn-success"><i className="fa fa-search"></i></button>
        </div>
