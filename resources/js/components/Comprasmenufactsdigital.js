export default function Comprasmenufactsdigital({
    ...
    return <div className="text-center">
        <div className="btn-group mb-2">
            ...
            <button className={("fs-2 btn btn")+(viewmainPanel=="distribuirfacts"?"":"-outline")+("-sinapsis")} onClick={()=>setviewmainPanel("distribuirfacts")}>DISTRIBUIR <span className="badge bg-secondary">4</span></button>
            <button className={("fs-2 btn btn")+(viewmainPanel=="pedidos"?"":"-outline")+("-sinapsis")} onClick={()=>setviewmainPanel("pedidos")}>PEDIDOS <span className="badge bg-secondary">5</span></button>
        </div>
    </div>
}